...
/* 
2.	Next, it would great to see a similar monthly trend for Gsearch, but this time splitting out nonbrand
...
/*
4. 	I'm worried that one of our pessimistic board members may be concerned about the large % of traffic from Gsearch.
...
-- first find the minimum pageview_id to find out when it started
...
-- .0320 for /home vs .0423 for /lander-1
-- .0743 additional orders per session
...
-- find how many sessions we have since that test
SELECT
...
SELECT 
    ...
    CASE WHEN pageview_url = '/lander-1' THEN 1 ELSE 0 END AS custom_lender,
...
-- CREATE TEMPORARY TABLE session_level_made_it_flaged
SELECT
	website_session_id,
...
FROM session_level_made_it_flaged
GROUP BY 1
;
    

-- part 2 conversion rate %
